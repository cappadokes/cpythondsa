...
PYTHONMALLOC=malloc LD_PRELOAD=$CODE/jemalloc/lib/libjemalloc.so.2 python3 -m pyperformance run --rigorous --inherit-environ LD_PRELOAD,PYTHONMALLOC,LD_LIBRARY_PATH --venv $VENV -o $RESULTS/jemalloc/$MACHINE/time.json.gz

# pymalloc-jemalloc.
$VENV/bin/python -m pyperf system tune
LD_PRELOAD=$CODE/jemalloc/lib/libjemalloc.so.2 python3 -m pyperformance run --rigorous --inherit-environ LD_PRELOAD,LD_LIBRARY_PATH --venv $VENV -o $RESULTS/pymalloc_jemalloc/$MACHINE/time.json.gz
...
# Jemalloc.
$VENV/bin/python -m pyperf system tune
PYTHONMALLOC=malloc LD_PRELOAD=$CODE/jemalloc/lib/libjemalloc.so.2 python3 -m pyperformance run --rigorous --inherit-environ LD_PRELOAD,PYTHONMALLOC,LD_LIBRARY_PATH --track-memory --venv $VENV -o $RESULTS/jemalloc/$MACHINE/mem.json.gz

# pymalloc-jemalloc.
$VENV/bin/python -m pyperf system tune
LD_PRELOAD=$CODE/jemalloc/lib/libjemalloc.so.2 python3 -m pyperformance run --rigorous --inherit-environ LD_PRELOAD,LD_LIBRARY_PATH --track-memory --venv $VENV -o $RESULTS/pymalloc_jemalloc/$MACHINE/mem.json.gz
